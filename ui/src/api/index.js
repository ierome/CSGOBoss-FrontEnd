...
function getChatHistory(req, res) {
  ...
  co(function* () {
    ...
    const history = yield ChatHistory
      .between([ channel, r.now().sub(1200) ], [ channel, r.maxval ], { index: 'chatChannelCreatedAt' })
      .orderBy({ index: r.asc('chatChannelCreatedAt') })
      .limit(40)
...
function getPlayerStats(req, res) {
  co(function* () {
    ...
    res.json({
      ...
      totalTokensTipped: player.totalTokensTipped || 0,
    })
  })
...
function postSend(req, res) {
  ...
  if(!is.string(to) || !is.number(req.body.amount) || req.body.amount <= 0) {
  ...
  const amount = Math.floor(req.body.amount * 100) / 100
  ...
  co(function* () {
    ...
    const { replaced, changes } = yield takePlayerBalance(req.user.id, amount, {
      ...
      extraUpdate: player => ({
        totalTokensTipped: player('totalTokensTipped').default(0).add(amount)
      })
    }, { })
